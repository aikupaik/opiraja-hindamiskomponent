...
describe('player demo', () => {
  it('creates through the OR boundary, exposes the link, and renders completed feedback', async () => {
    ...
    expect(screen.queryByRole('img', { name: 'Testimängija QR-kood' })).not.toBeInTheDocument()
    fireEvent.click(screen.getByRole('button', { name: 'Loo QR-kood telefoniga avamiseks' }))
    expect(screen.getByRole('img', { name: 'Testimängija QR-kood' })).toBeInTheDocument()
    expect(screen.getByText('Skanni kood telefoniga, et avada testimängija.')).toBeInTheDocument()
