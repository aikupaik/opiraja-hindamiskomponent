...
export function TopBar({ activeArea, operator, onLock }: { activeArea: AdminAreaId; operator: string; onLock: () => void }) {
  return <header className={styles.topBar}>
    <a className={styles.brand} href={hrefFor('materials')}><span className={styles.brandAccent} aria-hidden="true" /><span><strong>Hindamislabor</strong><small>Operaatori vaade</small></span></a>
    ...
    <div className={styles.operator}><span title="Aktiivne operaator"><i aria-hidden="true" />{operator}</span><Button variant="light" leadingIcon={<LockIcon />} type="button" onClick={onLock}>Lukusta</Button></div>
  </header>
}
