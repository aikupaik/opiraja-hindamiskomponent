...
| Supabase only | Existing repository operations against run-owned data | Concurrency 1-32; measure graph/session reads, per-node inventory listing, batched usable-pool and historical-review loads, answer commits, atomic usage-telemetry RPCs, and same-item telemetry contention |
...
| API to Supabase | Real API/repository with deterministic R fake | Vary graph and pool size; verify one usable-item batch per accepted answer, one historical-review batch at completion, and the absence of item-bank N+1 reads |
