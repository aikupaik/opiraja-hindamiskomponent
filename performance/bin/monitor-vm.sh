...
if [ "$#" -ne 1 ]; then
  echo "usage: bash performance/bin/monitor-vm.sh <run-id>" >&2
  exit 64
fi
...
docker compose ps > "$EVIDENCE_DIR/compose-ps-before.txt"

docker compose logs --tail 0 --follow --timestamps --no-color \
...
cleanup() {
  ...
  docker compose ps > "$EVIDENCE_DIR/compose-ps-after.txt" 2>&1 || true
